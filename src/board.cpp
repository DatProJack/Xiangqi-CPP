...
bool Board::takeable(int x, int y, char color) {
...
set<pair<int, int>> Board::getMoves(int x, int y) {
...
bool Board::over() {
    //see if it's over!
    //for the turn player, see if they have 0 legal moves. then check if their king is in check
    auto moves = getLegalMoves(turn);
    if(moves.size() != 0) {
        return false;
    }
    //check for checkmate or stalemate. if the 'turn' king is in check -> checkmate. else -> stalemate
    for(int i = 0; i < 10; i++) {
        for(int j = 0; j < 9; j++) {
            if(board[i][j][0] == turn) {
                if(inCheck(i, j, i, j)) {
                    cout << "Game over! " << (turn == 'r' ? "Black won!\n" : "Red won!\n");
                    return true;
                }
                break;
            }
...
bool Board::inCheck(int x1, int y1, int x2, int y2) {
...
bool Board::soldiercheck(int x, int y, char color) {//fix up these turn things
    if(board[x][y][0] == color) return false;
    char c = board[x][y][1];
    return c == 'S';
}
    
bool Board::directcheck(int x, int y, char color) {
    if(board[x][y][0] == color) return false;
    char c = board[x][y][1];
    return c == 'R' || c == 'G';
}

bool Board::cannoncheck(int x, int y, char color) {
    if(board[x][y][0] == color) return false;
    char c = board[x][y][1];
    return c == 'C';
}

vector<vector<int>> Board::getLegalMoves(char player) {
...
string Board::trymove(int x1, int y1, int x2, int y2) {
...
void Board::undomove(int x1, int y1, int x2, int y2, string captured) {
    board[x1][y1] = board[x2][y2];
    board[x2][y2] = captured;
}

void Board::finalMove(int x1, int y1, int x2, int y2) {
...
bool Board::isValidMove(int x1, int y1, int x2, int y2) {//only for player input
...
//int movecount = 0;
//int cachedmoves = 0;
int Board::minimax(int depth, int alpha, int beta, char player) {
    //movecount++;
    uint64_t hash_value = zhash.hash(board);
    int cached_score = ttable.lookup(hash_value, depth);
    if(cached_score != INT_MIN) {
        //cachedmoves++;
        return cached_score;
    }

    int score = evaluateBoard();
    ttable.store(hash_value, score, depth);
...
void Board::AIMove() {
    ...
    /*
    if(asdf.size() == 0) {//check to see if its checkmate or stalemate
        if(INCHECK) {
            checkmate
        }
        else STALEMATE
...
int Board::MoveGenerationTest(int depth, char color) {
    if(depth == 0) return 1;
    auto moves = getLegalMoves(color);
    int ans = 0;

    for(auto v : moves) {
        string captured = trymove(v[0], v[1], v[2], v[3]);
        ans += MoveGenerationTest(depth - 1, color == 'r' ? 'b' : 'r');
        undomove(v[0], v[1], v[2], v[3], captured);
    }
